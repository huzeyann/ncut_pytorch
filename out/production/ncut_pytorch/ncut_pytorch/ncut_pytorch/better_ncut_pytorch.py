...
class LaplacianKernel(OnlineKernel):
    ...
    def fit(self, features: torch.Tensor) -> None:
        ...
        self.Ainv = torch.inverse(self.A)                       # [n x n]
        print("jkl;", Ain)
    ...
    def update(self, features: torch.Tensor) -> torch.Tensor:
        ...
        print("asdf", torch.any(torch.isnan(B) | torch.isinf(B)))
        ...
        print("uiop", rowscale.abs().min().item(), rowscale.abs().max().item(), colscale.abs().min().item(), colscale.abs().max().item())
        scale = (rowscale[:, None] * colscale) ** -0.5          # [n x m]
        print("qwer", torch.any(torch.isnan(scale) | torch.isinf(scale)))
...
class BetterNCUT(OnlineNystrom):
    ...
    def fit(
        ...
        Returns:
            (NCUT): self
        """
        BetterNCUT._fit_helper(self, features, precomputed_sampled_indices)
        return self
    ...
    def fit_transform(
        ...
        """
        unsampled_indices, V_unsampled = BetterNCUT._fit_helper(self, features, precomputed_sampled_indices)
