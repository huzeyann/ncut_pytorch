...
class TestNcutDeterministic:
    ...
    def test_deterministic_with_different_parameters(self, large_feature_matrix):
        ...
        torch.manual_seed(seed)
        np.random.seed(seed)
        eigvec1, eigval1 = ncut_fn(large_feature_matrix, n_eig=5, quantile_sigma=0.2, n_sample=50)
        
        torch.manual_seed(seed)
        np.random.seed(seed)
        eigvec2, eigval2 = ncut_fn(large_feature_matrix, n_eig=5, quantile_sigma=0.2, n_sample=50)
